...
router.get('/all', (req, res) => {
    ...
    Profile.find()
        ...
        .then(profiles => {
            if (!profiles) {
                errors.noprofile = 'There are no profiles. forsenDED'
                return res.status(404).json(errors);
            }
            res.json(profiles)
        })
        .catch(err => res.status(404).json({ profile: 'There are no profiles. forsenDED' }));
})
...
router.post('/', passport.authenticate('jwt', { session: false }), (req, res) => {
    ...
    if (req.body.gitgubusername) profileFields.gitgubusername = req.body.gitgubusername;
    //Handleing Skills input
...
// @desc Delete and experience from profile
// @access Private
