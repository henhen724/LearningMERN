...
router.get('/', (req, res) => {
    Post.find()
        ...
        .catch(err => res.status(404).json({ nopostfound: "No posts exists" }));
});
...
router.get('/:id', (req, res) => {
    Post.findById(req.params.id)
        ...
        .catch(err => res.status(404).json({ nopostfound: "No such post exists" }));
});
...
router.delete('/:id', passport.authenticate('jwt', { session: false }), (req, res) => {
    Profile.findOne({ user: req.user.id })
        .then(profile => {
            Post.findById(req.params.id)
                .then(post => {
                    //Check if this person owns this post
...
// @desc Delete a comment on a post with :id
// @access Private
